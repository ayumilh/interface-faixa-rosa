"use client";

import { useState, useEffect } from "react";
import {
  FaCity,
  FaCheckCircle,
  FaTimesCircle,
  FaMapMarkerAlt,
  FaTimes,
} from "react-icons/fa";

export default function TrocaDeCidade() {
  const [isModalOpen, setModalOpen] = useState(false);
  const [animatePrice, setAnimatePrice] = useState(false);

  useEffect(() => {
    const timer = setTimeout(() => {
      setAnimatePrice(true);
    }, 500);
    return () => clearTimeout(timer);
  }, []);

  const closeModal = (e) => {
    if (e.target.id === "modalBackground") {
      setModalOpen(false);
    }
  };

  return (
    <div className="flex flex-col items-center justify-center bg-white p-4 sm:p-6">
      {/* Título do Plano */}
      <div className="flex items-center space-x-2 sm:space-x-3 mb-4 sm:mb-6">
        <FaCity className="text-green-500 text-2xl sm:text-3xl" />
        <h3 className="text-xl sm:text-2xl font-extrabold text-gray-800">
          Troca de{" "}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-green-600">
            Cidade
          </span>
        </h3>
      </div>

      {/* Pontos de Listagem */}
      <p className="bg-green-50 text-green-700 font-semibold text-xs sm:text-sm px-3 py-1 rounded-full inline-block mb-6 sm:mb-8">
        Troque de cidade facilmente
      </p>

      {/* Benefícios */}
      <ul className="space-y-2 sm:space-y-3 text-gray-700 text-sm sm:text-base mb-6 sm:mb-8 max-w-full sm:max-w-md">
        <li className="flex items-start space-x-2">
          <FaCheckCircle className="text-green-500 w-4 h-4" />
          <span>Anúncio online na nova cidade</span>
        </li>
      </ul>

      {/* Limitações */}
      <ul className="space-y-2 sm:space-y-3 text-gray-700 text-sm sm:text-base mb-6 sm:mb-8">
        <li className="flex items-start space-x-2">
          <FaTimesCircle className="text-red-500 w-4 h-4" />
          <span>Sem galeria de fotos</span>
        </li>
        <li className="flex items-start space-x-2">
          <FaTimesCircle className="text-red-500 w-4 h-4" />
          <span>Sem stories</span>
        </li>
        <li className="flex items-start space-x-2">
          <FaTimesCircle className="text-red-500 w-4 h-4" />
          <span>Sem convênio</span>
        </li>
      </ul>

      {/* Preços com Animação */}
      <div className="text-center mb-4 sm:mb-6 mt-6">
        <p
          className={`text-gray-600 text-sm sm:text-base line-through ${
            animatePrice ? "animate-strike" : ""
          }`}
        >
          DE R$ 88,00
        </p>
        <p
          className={`text-lg sm:text-xl font-bold text-gray-800 ${
            animatePrice ? "animate-fade-in" : "opacity-0"
          }`}
        >
          POR R$ 80,00
        </p>
        <span className="bg-red-500 text-white text-xs sm:text-sm font-semibold px-2 py-0.5 rounded-full mt-1 inline-block">
          Taxa Única a cada troca
        </span>
      </div>

      {/* Botão Contratar */}
      <button className="w-full max-w-xs bg-gradient-to-r from-green-400 to-green-600 text-white text-sm sm:text-base font-bold py-2 rounded-full hover:opacity-90 transition duration-300 ease-in-out mb-3">
        Contrate o Plano
      </button>

      {/* Botão Mais Informações */}
      <div className="mt-2 sm:mt-3 text-center">
        <button
          onClick={() => setModalOpen(true)}
          className="text-green-600 text-sm font-medium underline hover:text-green-800 transition duration-300"
        >
          Mais informações
        </button>
      </div>

      {/* Modal Responsivo */}
      {isModalOpen && (
        <div
          id="modalBackground"
          onClick={closeModal}
          className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 px-4"
        >
          <div className="bg-white rounded-2xl p-6 max-w-xs sm:max-w-sm w-full mx-auto shadow-2xl relative">
            <button
              onClick={() => setModalOpen(false)}
              className="absolute top-3 right-3 text-gray-500 hover:text-gray-700"
            >
              <FaTimes className="w-5 h-5" />
            </button>

            {/* Conteúdo do Modal */}
            <div className="text-center">
              <FaMapMarkerAlt className="text-green-500 text-3xl sm:text-4xl mb-3" />
              <h4 className="text-lg sm:text-xl font-bold text-gray-800 mb-2">
                Seu anúncio na nova cidade
              </h4>
              <p className="text-sm text-gray-600 mb-3">
                Alavanque seu perfil exibindo seu anúncio na cidade desejada e
                atraia novos clientes.
              </p>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
