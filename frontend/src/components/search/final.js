...
export default function Rodape() {
  ...
  return (
    <footer className="bg-black py-12 text-gray-300 mt-8">
      <div className="container mx-auto px-6 md:px-12 lg:px-24 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-4">
          ...
          <ul className="space-y-2 text-sm">
            ...
            <li>
              <Link href="/termos" className="hover:underline text-gray-300">Política de Privacidade</Link>
            </li>
            <li>
              <Link href="https://new.safernet.org.br/denuncie" className="hover:underline text-gray-300">Denunciar Exploração Sexual</Link>
            </li>
          </ul>
        </div>
      </div>
      ...
      {/* Rodapé Inferior */}
      <div className="border-t border-black mt-8 pt-4 text-center text-gray-500 text-sm">
        <p>&copy; {new Date().getFullYear()} Faixa Rosa. Todos os direitos reservados.</p>
      </div>
      ...
      {showFAQ && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" onClick={closeModal}>
          <div className="bg-white w-11/12 max-w-2xl p-6 rounded-lg shadow-lg overflow-y-auto max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-2xl font-bold mb-4 text-center text-pink-500">Perguntas Frequentes</h2>
            <button
              onClick={closeModal}
              className="absolute top-[-6px] right-2 text-gray-500 hover:text-gray-700 font-bold"
            >
              &times;
            </button>
            <div className="space-y-4">
