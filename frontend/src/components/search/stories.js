...
export default function Stories() {
  ...
  const companionData = companions.find(c => c.userName === userInfo?.userName);
  ...
  const extraPlanAllowsStories = companionData?.subscriptions?.some(
  ...
  async function fetchStories() {
    try {
      ...
      if (selectedStory) {
        ...
        if (updatedGroup) setSelectedStory(updatedGroup);
  ...
  const renderMediaContent = () => {
    if (selectedStory) {
      ...
      if (mediaType === "video") {
        ...
        return <Image src={url} alt="Story" width={500} height={800} className="max-h-[80vh] w-auto object-contain" />;
      }
    }
    ...
    <div className="w-full py-4 px-4 bg-gray-50 shadow-md">
        ...
        {canPostStory && (
          <div className="w-20 flex-shrink-0 text-center">
            <label className="w-16 h-16 rounded-full p-[2px] mx-auto bg-gradient-to-r from-pink-400 via-pink-700 to-pink-300 flex items-center justify-center cursor-pointer hover:animate-spin-slow">
              <div className="w-full h-full rounded-full bg-white text-pink-500 text-3xl flex items-center justify-center">+</div>
              <input type="file" accept="image/*,video/*" className="hidden" ref={inputRef} onChange={handleStoryUpload} />
            </label>
            <span className="block mt-2 text-xs text-gray-700 font-medium max-w-[4.5rem] truncate mx-auto">Adicionar</span>
          </div>
        )}
        {stories.map((storyGroup, index) => (
          <div key={index} className="w-20 flex-shrink-0 text-center">
            <div className="w-14 h-14 rounded-full overflow-hidden border-4 border-gradient-to-r from-pink-500 to-purple-500 cursor-pointer hover:scale-105 transform transition-transform duration-300" onClick={() => openStory(index)}>
              {storyGroup.companion?.profileImage ? (
                <Image src={storyGroup.companion.profileImage || "/default-avatar.jpg"} alt={storyGroup.companion.userName || "Desconhecido"} width={56} height={56} className="w-full h-full object-cover" />
              ) : (
      ...
      {selectedStory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 backdrop-blur-sm transition-opacity duration-300">
          <div className="bg-gray-800 w-full max-w-sm md:max-w-2xl p-6 rounded-xl shadow-lg relative flex flex-col items-center max-h-[90vh] overflow-y-auto" ref={modalRef}>
            <div className="absolute top-0 left-0 h-1 bg-gradient-to-r from-pink-500 to-purple-500" style={{ width: `${progress}%` }}></div>
            {selectedStory?.companion?.userName === userInfo?.userName && (
              <button onClick={() => handleDeleteStory(selectedStory.stories[currentIndex]?.id)} className="absolute top-10 right-10 z-[9999] bg-red-600 hover:bg-red-700 text-white text-lg font-bold px-3 py-2 rounded-full shadow-lg transition-all">Deletar</button>
            )}
            <div className="absolute left-0 top-0 h-full w-1/2 cursor-pointer z-50" onClick={prevStory}></div>
            <div className="absolute right-0 top-0 h-full w-1/2 cursor-pointer z-50" onClick={nextStory}></div>
            <div ref={imageRef} className="flex-1 w-full flex items-center justify-center bg-black rounded-lg overflow-hidden cursor-pointer relative max-h-[80vh]" onClick={handleImageClick}>
              {renderMediaContent()}
            </div>
            <div className="mt-4 flex flex-col items-center" onClick={(e) => {
              e.stopPropagation();
              router.push(`/perfil/${slugify(selectedStory.companion?.userName)}`);
              closeModal();
            }}>
              {selectedStory.companion?.profileImage ? (
                <Image src={selectedStory.companion.profileImage} alt={selectedStory.companion?.userName || "Desconhecido"} width={56} height={56} className="w-14 h-14 object-cover rounded-full cursor-pointer" />
              ) : (
              ...
              <h3 className="text-xl md:text-2xl font-semibold text-white">{selectedStory.companion?.userName || "Desconhecido"}</h3>
              <p className="text-sm md:text-base text-gray-300 mt-2 text-center">{selectedStory.companion?.description}</p>
            </div>
          </div>
        </div>
      )}
